defmodule App.AI.Agent do
  ...
  defp build_system_message(context, instructions) do
    ...
    instructions_text = Enum.map_join(instructions, "\n", fn inst ->
      "- #{inst.instruction}"
    end)
  ...
  defp build_conversation_messages(messages, system_message) do
    ...
    conversation_msgs = Enum.map(messages, fn msg ->
  ...
  defp handle_ai_response(conversation, %{"tool_calls" => tool_calls} = ai_message, user) when is_list(tool_calls) do
    ...
    {:ok, assistant_msg} = Chat.create_message(conversation, %{
      role: "assistant",
      content: ai_message["content"] || "I'll help you with that.",
      metadata: %{tool_calls: tool_calls, tool_results: serialized_results}
    })
    ...
    if Enum.all?(tool_results, &match?({:ok, _}, &1)) do
      ...
      follow_up_messages = build_conversation_messages(
                           ...
                           ) ++ [%{role: "user", content: "Tool results: #{results_context}. Please provide a summary of what was accomplished."}]

      case OpenAI.chat_completion(follow_up_messages) do
        {:ok, %{"choices" => [%{"message" => %{"content" => summary}}]}} ->
          {:ok, summary_msg} = Chat.create_message(conversation, %{
            role: "assistant",
            content: summary
          })
          {:ok, summary_msg}
  ...
  defp execute_tool_call(%{"function" => %{"name" => tool_name, "arguments" => args_json}}, user) do
    try do
      ...
      %{
        ...
        function: %{
          ...
          parameters: %{
            type: "object",
            properties: %{
              contact_email: %{type: "string", description: "Email of person to meet with"},
      ...
      %{
        ...
        function: %{
          ...
          parameters: %{
            type: "object",
            properties: %{
              instruction: %{type: "string", description: "The instruction for automated behavior"},
              triggers: %{type: "array", items: %{type: "string"}, description: "When to trigger this instruction"}
            },
